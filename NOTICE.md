# Notice

This notice explains authorship, attribution, AI assistance, provenance, and responsibility boundaries for this repository.

## Authorship

Responsibility Pathway Engineering, as presented in this repository, is authored by:

Akihisa Ono (小野昭久)

The definitions, repository framing, specification direction, examples, and public claims in this repository are maintained under the responsibility of the human author.

## Provenance

The public source lineage and authorship traceability for **責任経路 / Responsibility Pathway** and **Responsibility Pathway Engineering** are recorded in [docs/provenance.md](docs/provenance.md).

That document is an attribution and traceability record for this repository.

## Copyright

Copyright (c) 2026 Akihisa Ono (小野昭久).

This repository is released under the MIT License. See [LICENSE](LICENSE).

## AI assistance

AI tools may have assisted with drafting, structuring, editing, implementation, consistency checks, and repository maintenance.

AI tools are not authors, copyright holders, legal responsibility holders, moral responsibility holders, or final decision-makers for this repository.

The human author remains responsible for deciding what is included, changed, published, claimed, or released.

## Reuse and attribution

Reuse, modification, distribution, and sublicensing are permitted under the MIT License, provided that the copyright notice and license notice are preserved.

When referring to the concept, definitions, examples, specifications, or provenance of this repository, attribution to the repository and author is appreciated, especially in public writing, research, standards discussion, implementation notes, or derivative documentation.

## Derivative interpretations

Derivative works, applications, implementations, explanations, or public claims based on this repository are the responsibility of their respective authors or operators.

This repository does not certify downstream systems, guarantee compliance, assign legal liability, resolve moral accountability, guarantee safety, guarantee fairness, or establish production readiness.

## Responsibility boundary

The project itself is operated as a Responsibility Pathway.

Claims should remain returnable to definitions, definitions to specifications, specifications to examples or formalization candidates, and formalization candidates to explicit assumptions and modeled scope.
