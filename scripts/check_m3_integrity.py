...
from rpe_kernel import evaluate_integrity_guarded_gateway_request
...
def main() -> int:
    ...
    integrated = evaluate_integrity_guarded_gateway_request(
        gateway_payload,
        integrity_checks=[
            {
                "check": {
    ...
    assert integrated["risk_condition_result"]["graph_status"] == "triggered", integrated
    assert integrated["risk_condition_result"]["triggered_conditions"] == ["integrity:approval-request-binding"], integrated
    ...
    integrated_unknown = evaluate_integrity_guarded_gateway_request(
        gateway_payload,
        integrity_checks=[
            {
                "check": {
    ...
    assert integrated_unknown["risk_condition_result"]["unknown_conditions"] == ["integrity:configuration-readback"], integrated_unknown
    assert integrated_unknown["transition_result"]["control_action"] == "hold", integrated_unknown

    competing = evaluate_integrity_guarded_gateway_request(
        {
            "contract_version": "0.1.0-exp",
            "governed_evaluation": base,
            "risk_graph": {
    ...
    missing_control = evaluate_integrity_guarded_gateway_request(
        gateway_payload,
        integrity_checks=[
            {
                "check": {
    ...
    print("M3 integrity comparison and additive gateway integration checks passed")
    return 0
