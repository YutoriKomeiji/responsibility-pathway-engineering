...
- Keep `docs/document-redundancy-review-plan.md` and `docs/document-redundancy-review-initial-pass-sync-note.md` available when choosing a post-initial-pass document-redundancy follow-up; the next step is selection from findings notes, not automatic merge, deletion, rename, or broad rewrite.
- Keep `docs/zenn-level-2-repository-walkthrough-readiness.md` available when repository-walkthrough publication readiness is discussed without reopening article drafting or title selection.
- Keep `docs/operation-index.md` aligned when operation documents, snapshots, sync logs, roadmap notes, progress map, focused connection notes, checker plans, runtime candidate planning, review notes, external-review readiness, external-review package notes, standardization strategy, document-role rules, concept navigation, social-connection review paths, Zenn Level 2 readiness, or AI local judgment task-control boundaries change.
...
- Keep `docs/phase-3-1-sync-log.md` as the detailed synchronization record for runtime-event, runtime-fixture, runtime-event checker stub, runtime-event workflow observation, minimal runtime fixture checker workflow observation, runtime-event schema/fixture alignment, event-to-pathway relation checker planning, external review readiness, external review package note, README recovery, missed-support synchronization, progress-map synchronization, focused connection-note synchronization, responsibility pathway availability synchronization, AI Judgment Node synchronization, social-connection review synchronization, Zenn Level 2 readiness synchronization, and future standardization synchronization units.
...
- Keep `docs/zenn-level-2-repository-walkthrough-readiness.md` as the repository-side readiness note for Level 2 repository walkthrough publication, without treating it as an article draft, title decision, external review result, or implementation claim.
...
- `docs/zenn-level-2-repository-walkthrough-readiness.md` records repository-side Level 2 readiness for a Zenn repository walkthrough without deciding article drafting or title selection.
...
- Keep `docs/evidence-approval-transfer-alignment.md` available when reviewers need to inspect approval skip, evidence preservation, transfer destination, and return-path visibility.
...
5. P1: keep Zenn Level 2 repository walkthrough readiness connected before treating repository-walkthrough publication as supported.
6. P1: keep AI Judgment Node task-control boundary connected before discussing loop-like task control, goal-like stop conditions, evaluator separation, or local AI judgment as Phase 3.1 runtime-event design inputs.
7. P1: consider `docs/phase-3-1-snapshot-reduction-note.md` before adding more narrow detailed sections to `docs/phase-3-1-current-snapshot.md`.
8. P1: keep progress map rough, planning-only, and connected before using progress percentages in public-facing language.
9. P1: keep `docs/phase-3-1-progress-map-connection.md` available when Phase 3.1 progress visibility is needed without rewriting the full current snapshot.
10. P1: keep standardization strategy grounded and connected before any public-facing world-standard language is expanded.
11. P1: keep external review package note, external review readiness checklist, event-to-pathway relation checker plan, missed-support current status, example index, checker coverage, social-connection review navigation, repository pathway gap inventory, Zenn Level 2 readiness, and focused reader paths aligned after review-readiness, package-note, checker-planning, social-connection review, or publication-readiness changes.
