# Expanded README Archive

This document preserves the expanded README content that was moved out of the repository root README after the GitHub mobile app appeared to render the root README as a blank page.

The root `README.md` should remain a short entry point. Detailed reader paths should live in focused documents such as `docs/operation-index.md`, `docs/overview.md`, `docs/example-index.md`, `docs/checker-coverage.md`, and this archive.
...
This repository is intentionally small and specification-first. See [docs/minimal-core-rationale.md](minimal-core-rationale.md) for why the current core prioritizes definitions, examples, lifecycle boundaries, checker boundaries, and excluded claims before larger implementation layers.

A source-alignment pass over the currently inventoried Zenn article candidates has been completed. The synthesis is available at [docs/source-alignment/zenn-source-alignment-synthesis.md](source-alignment/zenn-source-alignment-synthesis.md). The detailed inventory is available at [docs/source-alignment/zenn-article-inventory.md](source-alignment/zenn-article-inventory.md).

A developer-facing explanatory note for the try/catch/finally analogy is available at [docs/explanations/try-catch-finally-responsibility-pathway.md](explanations/try-catch-finally-responsibility-pathway.md). It is an explanatory analogy only, not formal semantics.

A repository overview after the source-alignment pass is available at [docs/overview.md](overview.md).
...
The Phase 2 Lean spine has now been split into small modules while preserving `Core.lean` as the stable entry point. See [formal/lean/README.md](../formal/lean/README.md), [docs/phase-2-current-snapshot.md](phase-2-current-snapshot.md), [docs/phase-2-lean-split-plan.md](phase-2-lean-split-plan.md), and [docs/phase-2-lean-theorem-index.md](phase-2-lean-theorem-index.md).

An initial enterprise adoption guide is available at [docs/enterprise-implementation-profile.md](enterprise-implementation-profile.md). It describes how organizations can connect the minimal formal core to workflow, evidence, checker, and governance layers without treating RPE as a production verifier, compliance engine, safety certification tool, or legal decision system.
...
Phase 3 has an entry boundary at [docs/reference-implementation-boundary.md](reference-implementation-boundary.md). Read it before adding reference implementations.
...
Phase 3.1 has started with an adapter boundary and runtime-event bridge. See [docs/adapter-boundary.md](adapter-boundary.md) and [docs/phase-3-1-current-snapshot.md](phase-3-1-current-snapshot.md). The first runtime-event-to-pathway example is [examples/runtime-event-to-pathway-minimal.yaml](../examples/runtime-event-to-pathway-minimal.yaml). It remains a synthetic, vendor-neutral, review-required draft example, not a service-specific connector or production runtime.

The repository operation model is available at [docs/repository-operation-model.md](repository-operation-model.md). It documents staged update operation, snapshots, sync logs, roadmap notes, checker observation policy, long-file update policy, and non-certifying operation boundaries.

The operation document index is available at [docs/operation-index.md](operation-index.md). It explains which operation-related document to read for each maintenance situation.
...
2. [docs/repository-operation-model.md](repository-operation-model.md) - repository operation model for staged updates, snapshots, sync logs, and long-file update policy
3. [docs/operation-index.md](operation-index.md) - navigation index for operation-related documents
4. [docs/overview.md](overview.md) - current repository overview after source alignment
5. [docs/source-alignment/zenn-source-alignment-synthesis.md](source-alignment/zenn-source-alignment-synthesis.md) - source-alignment synthesis from reviewed Zenn materials
6. [docs/concepts/core-elements.md](concepts/core-elements.md) - operational roles and controls aligned with the eight-element model
7. [docs/action-class-matrix.md](action-class-matrix.md) - source-aligned Class A-F action classification design aid
...
See [docs/example-index.md](example-index.md) for example purposes, boundaries, and future example candidates.
...
See [docs/validator-boundary.md](validator-boundary.md) for the checker boundary and [docs/checker-coverage.md](checker-coverage.md) for the current checker coverage map.
...
The current Phase 2 snapshot is available at [docs/phase-2-current-snapshot.md](phase-2-current-snapshot.md).

The theorem-role index is available at [docs/phase-2-lean-theorem-index.md](phase-2-lean-theorem-index.md).
