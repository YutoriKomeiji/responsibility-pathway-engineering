...
境界付きの確認結果 schema は [spec/review-result.schema.yaml](spec/review-result.schema.yaml) にあります。この schema は、確認結果が何を確認し、何を確認しておらず、何を主張していないかを記録するための公開仕様です。現在の review-result fixture は、まだチェッカーによる検証対象ではありません。
...
Review-result output fixture は pathway example とは分けて `fixtures/review-results/` に置かれています。これらは説明用の出力 fixture であり、現在は `scripts/check_examples.py` による検証対象ではありません。
...
`fixtures/review-results/` にある review-result fixture は、現在このチェッカーでは読み込まれません。`spec/review-result.schema.yaml` は公開 schema 境界であり、完了済みの checker integration ではありません。
